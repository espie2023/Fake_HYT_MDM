...
### 安全特性
- **隐私保护**：未登录用户只能查看基本信息（呼号、型号、更新时间），GPS位置、地图、硬件详情等敏感信息需登录后查看
- **访问控制**：基于 Cookie 的会话管理，支持登录/登出功能
- **权限管理**：管理员可控制用户访问特定设备或所有设备
- **安全认证**：关键操作需要设备SN验证或管理员权限
- **位置隐私**：GPS坐标精度降低到小数点后2位（约1.1公里），保护精确位置信息
...
浏览器访问 `http://your-server-ip:2232`

**未登录状态**：可以查看所有设备的基本信息（呼号、型号、更新时间）

**登录方式**：
- 点击页面右上角的"登录"按钮
- 输入用户名和密码（密码需要SHA-256哈希）
- 登录成功后可查看设备完整信息和GPS位置
...
### v1.1.0 (2025-01-07) - 安全增强版本

**新增功能**：
- 添加登出功能，支持用户安全退出登录
- 未登录状态下的访客模式，允许查看基本设备信息

**安全改进**：
- 强化隐私保护：未登录用户无法查看GPS位置、地图、IP/MAC地址等敏感信息
- 移除公开注册功能：防止未授权用户自行注册账号
- GPS位置精度降低：保护设备精确位置（精度约1.1公里）
- 改进访问控制：关键信息和功能需要登录后访问
